...
@implementation YDFMResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(YDFMStatement *)statement usingParentDatabase:(YDFMDatabase*)aDB {
    
    YDFMResultSet *rs = [[YDFMResultSet alloc] init];
    ...
    return YDFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    YDFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return YDFMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"YDFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (void)setParentDB:(YDFMDatabase *)newDb {
    _parentDB = newDb;
}
