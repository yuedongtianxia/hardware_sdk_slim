...
/** Represents the results of executing a query on an `<YDFMDatabase>`.
...
@interface YDFMResultSet : NSObject {
    YDFMDatabase          *_parentDB;
    YDFMStatement         *_statement;
...
/** `YDFMStatement` used by result set. */

@property (atomic, retain) YDFMStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<YDFMStatement>`
 
 @param statement A `<YDFMStatement>` to be performed
 
 @param aDB A `<YDFMDatabase>` to be used
 
 @return A `YDFMResultSet` on success; `nil` on failure
 */
