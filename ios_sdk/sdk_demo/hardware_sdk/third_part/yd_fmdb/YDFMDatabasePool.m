...
@interface YDFMDatabasePool()

- (void)pushDatabaseBackInPool:(YDFMDatabase*)db;
- (YDFMDatabase*)db;

@end


@implementation YDFMDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return YDFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = YDFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = YDFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    YDFMDBRelease(_path);
    YDFMDBRelease(_databaseInPool);
    YDFMDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        YDFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(YDFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The YDFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (YDFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [YDFMDatabase databaseWithPath:self->_path];
