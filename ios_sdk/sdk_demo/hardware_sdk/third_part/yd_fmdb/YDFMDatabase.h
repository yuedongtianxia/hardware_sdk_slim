...
#import "YDFMResultSet.h"
#import "YDFMDatabasePool.h"


#if ! __has_feature(objc_arc)
    #define YDFMDBAutorelease(__v) ([__v autorelease]);
    #define YDFMDBReturnAutoreleased YDFMDBAutorelease

    #define YDFMDBRetain(__v) ([__v retain]);
    #define YDFMDBReturnRetained YDFMDBRetain

    #define YDFMDBRelease(__v) ([__v release]);

    #define YDFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define YDFMDBAutorelease(__v)
    #define YDFMDBReturnAutoreleased(__v) (__v)

    #define YDFMDBRetain(__v)
    #define YDFMDBReturnRetained(__v) (__v)

    #define YDFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define YDFMDBDispatchQueueRelease(__v)
    #else
        #define YDFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^YDFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in YDFMDB are:

 - `YDFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<YDFMResultSet>` - Represents the results of executing a query on an `YDFMDatabase`.
 - `<YDFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<YDFMDatabasePool>` - A pool of `YDFMDatabase` objects.
 - `<YDFMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [YDFMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [YDFMDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `YDFMDatabase` object and use it across multiple threads. Instead, use `<YDFMDatabaseQueue>`.
...
@interface YDFMDatabase : NSObject  {
...
/** Create a `YDFMDatabase` object.
 
 An `YDFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `YDFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `YDFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    YDFMDatabase *db = [YDFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    YDFMDatabase *db     = [YDFMDatabase databaseWithPath:dbPath];
 ...
 @return `YDFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `YDFMDatabase` object.
...
/** Execute single update statement
 ...
 @note If you want to use this from Swift, please note that you must include `YDFMDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeUpdate:withArgumentsInArray:>`.
 */
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(YDFMDBExecuteStatementsCallbackBlock)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<YDFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[YDFMResultSet next]>`) from one record to the other.
 ...
 @return A `<YDFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see YDFMResultSet
 @see [`YDFMResultSet next`](<[YDFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 
 @note If you want to use this from Swift, please note that you must include `YDFMDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.
 */

- (YDFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (YDFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (YDFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String!, values: [AnyObject]!) throws  -> YDFMResultSet!`
...
- (YDFMResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error;
...
- (YDFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (YDFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
+ (NSString*)YDFMDBUserVersion;

+ (SInt32)YDFMDBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(YDFMDatabase *adb) {
        ...
        int rowCount = 0;
        YDFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        YDFMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [YDFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one YDFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using YDFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to YDFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using YDFMDB you will not need to interact directly with `YDFMStatement`, but rather with `<YDFMDatabase>` and `<YDFMResultSet>` only.
...
@interface YDFMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
